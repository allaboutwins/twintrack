...
async function ensureHeaders(sheetName: string, headers: string[]): Promise<void> {
  ...
  try {
    const checkRes = await connectors.proxy(
      "google-sheet",
      `/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(sheetName + "!A1:A1")}`,
      { method: "GET" },
    );
    ...
    if (!checkData.values?.length) {
      await connectors.proxy(
        "google-sheet",
        `/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(sheetName + "!A1")}:append?valueInputOption=USER_ENTERED`,
...
export async function appendOnboardingRow(data: {
  ...
  if (!SPREADSHEET_ID) {
    logger.info("sheets: GOOGLE_SHEET_ID not set, skipping sync");
    return;
  }
  try {
    await ensureHeaders("Onboarding", ONBOARDING_HEADERS);
    const row = [
    ...
    const res = await connectors.proxy(
      "google-sheet",
      `/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(ONBOARDING_RANGE)}:append?valueInputOption=USER_ENTERED`,
      { method: "POST", body: JSON.stringify({ values: [row] }) },
    );
    if (!res.ok) {
      const text = await res.text();
      logger.warn({ status: res.status, text }, "sheets: append failed");
    } else {
      logger.info({ userId: data.userId }, "sheets: onboarding row appended");
    }
  } catch (err) {
